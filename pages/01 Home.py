...
if on_perimetro.shape[0]!=0:
    ...
    data_week= dt_vaca['createdAt'].groupby(dt_vaca.createdAt.dt.isocalendar().day).aggregate(['count']).rename(columns={'count':'count_register'})
...
if int(data_week['count_register'].min())!= int(data_week['count_register'].max()):
        fig= px.bar( data_week,x=data_week.day,y='count_register')
